...
net.Server.prototype.listen = function() {
  ...
  if (typeof args[0] !== 'number') { throw new Error('No port Specified'); }
  // assume port. and host.
  options.port = args[0];
  options.host = ('string' === typeof args[1])
    ? args[1]
    : "127.0.0.1";

  options.backlog = ('number' === typeof args[1])
    ? args[1]
  ...
  _proxy.accepted = function (e) {
    self._accept({id: guid(), socket: e.source, _proxy:e.inbound });
  };
...
net.Server.prototype._accept = function(acceptInfo) {
  ...
  var self = this;
  var socket = new net.Socket({bind:acceptInfo._proxy});
...
net.Socket.prototype.connect = function() {
  ...
  self._proxy.connected = function (e) {
    ...
    if (e.bytesProcessed == -1) { self._proxy.close(); return self.emit('end'); }
    // TODO check if this is correct
    var _buffer = Buffer(e.buffer.toString());
...
net.Socket.prototype.setKeepAlive = function(enable, delay) {
  if ( !enable && !delay ) { return; }
  this._keepAliveTimer = setInterval(function (){ /*write keep alive to proxy*/ },delay);
};
...
net.Socket.prototype.write = function(data, encoding, fn) {
  ...
  self._resetTimeout();
  var bufferProxy = Ti.createBuffer({ value: _buffer });
