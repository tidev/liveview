...
const http = require('http'),
	...
	parsei18n = require('./i18n-parser').parsei18n,
	util = require('util'),
...
function getPids() {
	return fs.readdirSync(TMP_DIR)
		.filter(file => pidRegexp.test(file))
...
FServer.stop = function (env) {
	...
	pids.forEach(function (pidPath) {
		try {
			let _pid = path.basename(pidPath).split('-')[0];
			fs.removeSync(pidPath);
			log(`Closing file/event server process id: ${_pid}`);
			process.kill(_pid);
		} catch (e) {
			logError(`Error closing server ${e.stack}`);
		}
	});
...
FServer.start = function (opts) {
	...
	const fServer = http.createServer(function (request, response) {
		...
		if (uri === '/localeStrings.js') {
			...
			const langData = parsei18n(LOCALE_DIR);
			...
			try {
				...
				if (uri === '/app.js') {
					const scope = detect(fileContents);
					let shiftCount = 0;
					...
					v.forEach(function (vr) {
						...
						if (line.type === 'VariableDeclarator') {
							...
							fileContents[start] = ' ';
							fileContents[start + 1] = ' ';
							fileContents[start + 2] = ' ';

							if (line.parent && line.parent.kind === 'const') {
								fileContents[start + 3] = ' ';
								fileContents[start + 4] = ' ';
							}

							if (line.init === null) {
								fileContents.splice(line.end + shiftCount, 0, ' = null');
								shiftCount++;
							}
						}
					});
				...
				out.data = fileContents;
			} catch (err) {
				out.code = 500;
				out.data = `${err.message} ${err.stack}`;
			}
	 ...
	 */
	if (hasLocale) {
		const localeWatcher = evtServer.watcher = fsWatcher.watch(LOCALE_DIR, { persistent: true, ignoreInitial: true, ignored: 'alloy_generated' });
		...
		localeWatcher.on('change', handleChange)
			.on('add', handleChange)
	...
	if (isAlloy) {
		...
		function buildAlloy(file, eventType) {
			if (isBuilding) {
				return log('File changes ignored while Alloy compiling');
			}
			isBuilding = true;
			log(`Alloy recompile initiated for ${PLATFORM}`);
			const cmd = [ 'compile', ALLOY_DIR, '--no-colors' ];
			let config = `--config platform=${PLATFORM}`;
			// Only call a partial compile when a file is changed
			// not when a file is added or removed
			if (file && eventType === 'change') {
				// Alloy expects the file to be passed in with like the below
				// file=app/controllers/index.js
				config = `${config},file=${path.relative(PROJECT_DIR, file)}`;
			}
			cmd.push(config);
			let alloyCmd = 'alloy';
			...
			debug('alloy compile command %s, args %o', alloyCmd, cmd);
			const child = spawn(alloyCmd, cmd, { shell: true, windowsHide: true });
			let alloyOutput = '';

			child.stdout.on('data', (data) => {
				alloyOutput = `${alloyOutput} ${data.toString()}`;
			});
			...
			child.on('close', (code) => {
				isBuilding = false;
				if (code) {
					return logError(alloyOutput);
				}
				evtServer.emit('change', file);
			});
		}

		alloyWatcher.on('change', (file) => {
			buildAlloy(file, 'change');
		});

		alloyWatcher.on('add', (file) => {
			buildAlloy(file, 'add');
		});
	}
