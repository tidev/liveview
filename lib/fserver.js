...
FServer.start = function (opts) {
	...
	const fServer = http.createServer(function (request, response) {
		...
		const projectNodeModulesIndexPath = join(RESOURCE_DIR, requestPlatform, 'node_modules', uri, 'index' + urlPath.ext);
		const projectNodeModulesNoExtension = join(RESOURCE_DIR, requestPlatform, 'node_modules', uri);
