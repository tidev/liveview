...
FServer.start = function (opts) {
	...
	const fServer = http.createServer(function (request, response) {
		const urlPath = path.parse(request.url);

		const uri = join(urlPath.dir, urlPath.name);
		const requestPlatform = request.headers['x-platform'] || PLATFORM === 'ios' ? 'iphone' : PLATFORM;
		...
		const projectNodeModulesPath = join(RESOURCE_DIR, requestPlatform, 'node_modules', uri + urlPath.ext);
		const projectNodeModulesPath2 = join(RESOURCE_DIR, requestPlatform, 'node_modules', uri, 'index' + urlPath.ext);
		const projectNodeModulesPath3 = join(RESOURCE_DIR, requestPlatform, 'node_modules', uri);
		// This is used to handle the polyfill related modules as we don't
    ...
    */
			function next(i) {
				...
				fs.createReadStream(join(LOCALE_DIR, langs[i], 'strings.xml')).pipe(parser).on('data', function () {
			...
			log('[LiveView]'.blue, 'overridePath: ' + overridePath);
			log('[LiveView]'.blue, 'platformFilepath: ' + platformFilepath);
			log('[LiveView]'.blue, 'platformFilepath2: ' + platformFilepath2);
			log('[LiveView]'.blue, 'projectNodeModulesPath: ' + projectNodeModulesPath);
			log('[LiveView]'.blue, 'projectNodeModulesPath2: ' + projectNodeModulesPath2);
			log('[LiveView]'.blue, 'projectNodeModulesPath3: ' + projectNodeModulesPath3);
			log('[LiveView]'.blue, 'buildNodeModulesPath: ' + buildNodeModulesPath);

			if (fs.existsSync(overridePath)) {
				filename = overridePath;
			} else if (fs.existsSync(platformFilepath)) {
				filename = platformFilepath;
			} else if (fs.existsSync(platformFilepath2)) {
				filename = platformFilepath2;
			} else if (fs.existsSync(projectNodeModulesPath)) {
				filename = projectNodeModulesPath;
			} else if (fs.existsSync(projectNodeModulesPath2)) {
				filename = projectNodeModulesPath2;
			} else if (fs.existsSync(path.join(projectNodeModulesPath3, 'package.json'))) {
				const pkg = fs.readJsonSync(path.join(projectNodeModulesPath3, 'package.json'));
				if (pkg.main) {
					if (fs.existsSync(path.join(projectNodeModulesPath3, pkg.main))) {
						filename = path.join(projectNodeModulesPath3, pkg.main);
					}
				}
			...
			log('[LiveView]'.red, 'filename: ' + filename);
	...
	evtServer.on('connection', function (client) {
		...
		client.on('end', function () {
			...
			evtServer.clients = evtServer.clients.filter(cl => cl === client);

			// stop the server if no client reconnected in 15 minutes
			autoStopTimer = setTimeout(function () {
				if (evtServer.clients.length <= 1) {
					FServer.stop();
				}
			}, 1000 * 60 * 15);
		});
  ...
  */
	if (isAlloy) {
		...
		alloyWatcher.on('change', function (file) {
			...
			let alloyCmd = process.platform === 'win32' ? 'alloy.cmd' : 'alloy';
			if (process.env.ALLOY_PATH) {
				alloyCmd = `"${process.execPath}" ${process.env.ALLOY_PATH}`;
			}
