...
FServer.start = function(opts) {
  ...
  var BIN_DIR = join(LV_DIR, '../bin');
	...
	var fServer = http.createServer(function(request, response) {
		...
		var requestPlatform = request.headers['x-platform'] || 'iphone';
    ...
    if (!~PLATFORMS.indexOf(requestPlatform)) { PLATFORMS.push(requestPlatform); }
		...
		if (uri === '/localeStrings.js') {
			if (!hasLocale) {
				response.write('module.exports = null;');
			  response.end();
			  return;
			}
	...
	evtServer.on('change', function(obj){
		var clients = evtServer.clients.slice(0);
		var clientLen = clients.length;

		logFsChange(obj.file, 'Changed');

		for (var i = 0; i < clientLen; i++) {
			try {
				clients[i].write(JSON.stringify({type:'event', name:'reload', platform: (obj.platform || 'all') }));
	...
	resourceWatcher.on('change', function (file) {
		...
		evtServer.emit('change', {file: file});
	});
			...
			for(var i = 0; i <  PLATFORMS.length; i ++) {
				(function(_i){
					...
					exec('alloy compile --no-colors --config platform=' + platform, {silent: true}, function (code, error) {
						...
						evtServer.emit('change', { file: file, platform: platform });
					});
				})(i);
			}
		});
