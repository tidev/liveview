	...
	const fServer = http.createServer(function (request, response) {
		...
		if (uri === '/') {
			...
			*/
			...
			} else if (fs.existsSync(projectNodeModulesMainPath)) {
				filename = projectNodeModulesMainPath;
			} else if (fs.existsSync(projectNodeModulesIndexPath)) {
				filename = projectNodeModulesIndexPath;
			} else if (fs.existsSync(path.join(projectNodeModulesNoExtension, 'package.json'))) {
				const pkg = fs.readJsonSync(path.join(projectNodeModulesNoExtension, 'package.json'));
				if (pkg.main) {
					if (fs.existsSync(path.join(projectNodeModulesNoExtension, pkg.main))) {
						filename = path.join(projectNodeModulesNoExtension, pkg.main);
					}
				}
